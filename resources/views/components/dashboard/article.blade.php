@if(count($articles) == 0)
    <div class="card text-center">
        <div class=" text-center p-2">
           <h5> با نام شما مقاله ای به ثبت نرسیده است</h5>
        </div>
        @if(auth()->user()->IsAdmin() || str_contains(auth()->user()->roles , 'Write-Article'))
            <div class="card-body">
            <a href="{{route('article.create')}}" class="btn btn-primary">ایجاد مقاله جدید</a>
            </div>
        @else
        <div class="card-body">
            <h5 class="card-title">ایجاد مقاله</h5>
            <p class="card-text">اگر علاقه به ایجاد مقاله در زمینه های مختلف دارید میتوانید به کلیک کردن دکمه اجازه
                دسترسی را بگیرید.</p>
    ...
    <div class="row g-6 g-xl-9 ">
        @foreach($articles as $article)
            ...
            <div class="col-md-6 col-xl-5">
                <!--begin::Card-->
                <a href="{{route('article.show' , $article->slug)}}" class="card border-hover-primary">
                    <!--begin::Card header-->
                    <div class="card-header border-0 pt-9">
                        ...
                        <div
                            ...
                            <div class="menu-item px-3">
                                <a href="{{route('article.edit' , $article->slug)}}" class="menu-link px-5 border-0">ویرایش</a>
                            </div>
                            <!--end::Menu item-->
                            <!--begin::Menu item-->
                            <div class="menu-item px-3">
                    ...
                    <div class="card-body p-9">
                        <!--begin::Name-->
                        <div class="fs-3 fw-bolder text-dark text-right">{{$article->name}}</div>
                        <!--end::Name-->
                        <!--begin::Description-->
                        <p class="text-gray-400 fw-bold fs-5 mt-1 mb-7 text-right">{{substr($article->description , 0, 50).'.....'}}</p>
                        <!--end::Description-->
                        <!--begin::Info-->
                        <div class="d-flex flex-wrap mb-5">
                            <!--begin::Due-->
                            <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-7 mb-3">
                                <div class="fs-6 text-gray-800 fw-bolder">{{jdate($article->created_at)}}</div>
                                <div class="fw-bold text-gray-400">تاریخ ایجاد</div>
                            </div>
                            <!--end::Due-->
                            <!--begin::Budget-->
                            <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 mb-3">
                                <div class="fs-6 text-gray-800 fw-bolder">{{count($article->comments)}}</div>
                                <div class="fw-bold text-gray-400">تعداد نظرات</div>
                            </div>
                            <!--end::Budget-->
                        </div>
                        <!--end::Info-->
                        @if(count($article->favorites) > 0)
                            <!--begin::Progress-->
